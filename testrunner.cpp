...
void TestRunner::add(const QString &path) { add(QVariant(path)); }
...
bool TestRunner::exec(QStringList arguments) {
  ...
  foreach (item, m_testObjects) {
    ...
    } else if (item.typeId() == (int)QMetaType::QString) {
...
void TestRunner::addImportPath(const QString &path) { m_importPaths << path; }

int TestRunner::count() const { return m_testObjects.size(); }

void TestRunner::add(QVariant value) { m_testObjects.append(value); }
...
bool TestRunner::run(QObject *object, const QStringList &arguments) {
  ...
  execute = args.size() > 0 ? false : true; // If no argument passed , it should
                                            // always execute a test object
...
bool TestRunner::run(QString path, const QStringList &arguments) {
  ...
  char **s =
      (char **)malloc(sizeof(char *) * (10 + args.size() + paths.size() * 2));
  int idx = 0;
  ...
  foreach (QString arg, args) { s[idx++] = strdup(arg.toUtf8().data()); }
  s[idx++] = 0;
...
QVariantMap TestRunner::config() const { return m_config; }

void TestRunner::setConfig(const QVariantMap &config) { m_config = config; }
...
QStringList TestRunner::arguments() const { return m_arguments; }

TestRunner *TestRunner::defaultInstance() { return m_defaultInstance; }
